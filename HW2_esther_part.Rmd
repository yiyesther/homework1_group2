...
week_plot <- actual_bike_pw %>% 
  ...
  scale_x_continuous(breaks = seq(1,53,by=1), limits = c(0,53))+
  theme(axis.ticks = element_line(colour = "black", size = 2))+ 
  facet_wrap(~year, scales = "free")+
  theme_minimal()+
  theme(axis.ticks.length=unit(-0.25, "cm"))+
  theme(axis.ticks.x = element_line(color = x_axis_color, size = 0.3), axis.text.x = element_text(size=0)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  labs(title = "Monthly changes in TfL bike rentals", subtitle = "Change from monthly average shown in blue
  and calculated between 2016-2019",y= "", x = "week")
