...
prop_gss %>% 
  group_by(prop) %>% 
  summarise(mean_delta = mean(prop),
            sd_delta = sd(prop, na.rm=TRUE),
            count_delta = n(),
            se_delta = sd_delta / sqrt(count_delta),
            ci_delta_up = mean_delta + qt(.975, count_delta-1)*se_delta ,
            ci_delta_dw = mean_delta - qt(.975, count_delta-1)*se_delta 
...
actual_bike_pw <- bike %>% 
  ...
  pivot_longer(cols = 3:4, names_to = "Rental", values_to = "year")
...
actual_bike_pcm %>% 
  ggplot(aes(x = month)) +
  geom_line(aes(y = actual_rental, group = 1), color = "black")+
  geom_line(aes(y = expected_rental, group = 1), color = "blue", size = 0.7)+
  geom_ribbon(aes(ymin = actual_rental,ymax = expected_rental), fill="red", alpha=0.5) +
  facet_wrap(~year)
