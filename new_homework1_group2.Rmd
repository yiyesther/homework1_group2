...
vaccination_data <- vaccinations %>% 
  ...
  select(fips, recip_county, series_complete_yes,series_complete_pop_pct) %>% 
  arrange(fips)

vaccination_pct <- left_join(vaccination_data,population, by = "fips") 
...
vac_pop <- inner_join(vaccination_pct, vote_data, by = "fips") 
#vac_pop$vaccination_rate <- percent(vac_pop$vaccination_rate, accuracy=1)
#vac_pop$vote_rate <- percent(vac_pop$vote_rate, accuracy=1)
...
```{r,text}
vac_pop_plot + annotate("text", x = 0.1,y = 0.89, label = "Herd Immunity Threshold (?)", fontface = 'italic',color = 'blue',size = 3) +
...
The Guardian newspaper has an [election poll tracker for the upcoming German election](
...
```{r,challenge2_plot}
colors = c("Union" = "black", "SPD" = "red3", "AfD" = "midnightblue", "FDP" = "orange1",
...
election_data %>% 
  ...
  geom_line(aes(y = roll_mean_union, color="Union"), size=1) + 
  geom_line(aes(y = roll_mean_spd, color="SPD"),size=1) +
  geom_line(aes(y = roll_mean_af_d),size=1)+
  geom_line(aes(y = roll_mean_fdp),size=1)+
  geom_line(aes(y = roll_mean_linke),size=1)+
  geom_line(aes(y = roll_mean_grune),size=1)+
