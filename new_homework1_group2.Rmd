...
```{r,employee_info}
>>>>>>> 6cbfb9a38e747b451c09eb5b4540b0cad64b19f3
...
job_satis_dist <- hr_cleaned %>% 
  ...
  summarise(cnt = n()) %>%
<<<<<<< HEAD
  mutate(freq = percent (cnt / sum(cnt) * 100))
=======
...
job_satis_dist %>% 
  ...
  geom_bar(stat="identity")+
<<<<<<< HEAD
  labs(title = "job satisfaction", x = "job_satisfaction", y = "cnt")+
  NULL #to add the percentage as column "freq"
...
hr_cleaned %>% 
  ...
  labs(title = "Distribution of monthly income by education level", x = "Education", y = "Density") +
  NULL
...
Next, plot the vaccination rates less than 40% trump vote, less than 40% vaccinated.

```{r}
pct_trump_votes_vacc_pop %>%
  subset(percentage_trump_votes < 40 & series_complete_pop_pct < 40) %>% 
  ggplot(aes(x=percentage_trump_votes, y=series_complete_pop_pct, size=pop_estimate_2019)) + 
...
Next, plot the vaccination rates more than 60% trump vote, less than 40% vaccinated.

```{r}
pct_trump_votes_vacc_pop %>%
  subset(percentage_trump_votes >60 & series_complete_pop_pct < 40) %>% 
  ggplot(aes(x=percentage_trump_votes, y=series_complete_pop_pct, size=pop_estimate_2019)) + 
  geom_point(color = "cornflowerblue",
             alpha = .6) +
  scale_x_continuous(limits = c(0,100)) +
...
Next, plot the vaccination rates less than 40% trump vote, more than 60% vaccinated.

```{r}
pct_trump_votes_vacc_pop %>%
  subset(percentage_trump_votes < 40 & series_complete_pop_pct > 60) %>% 
  ggplot(aes(x=percentage_trump_votes, y=series_complete_pop_pct, size=pop_estimate_2019)) + 
  geom_point(color = "cornflowerblue",
             alpha = .6) +
  scale_x_continuous(limits = c(0,100)) +
...
Next, plot the vaccination rates more than 60% trump vote, more than 60% vaccinated.

```{r}
pct_trump_votes_vacc_pop %>%
  subset(percentage_trump_votes > 60  & series_complete_pop_pct > 60) %>% 
  ggplot(aes(x=percentage_trump_votes, y=series_complete_pop_pct, size=pop_estimate_2019)) + 
  geom_point(color = "cornflowerblue",
             alpha = .6) +
  scale_x_continuous(limits = c(0,100)) +
...
Next, plot the vaccination rates altogether with 40 - 60% range on either axis not shown.

```{r}
pct_trump_votes_vacc_pop %>%
  subset((percentage_trump_votes < 40 | percentage_trump_votes > 60)  & 
           (series_complete_pop_pct < 40 | series_complete_pop_pct > 60)) %>% 
  ggplot(aes(x=percentage_trump_votes, y=series_complete_pop_pct, size=pop_estimate_2019)) + 
  geom_point(color = "cornflowerblue",
             alpha = .6) +
  scale_x_continuous(limits = c(0,100)) +
...
vac_pop_plot <- vac_pop %>% 
  ...
  labs(title = "COVID 19 VACCINATION LEVELS OUT OF TOTAL POPULATION BY COUNTY", subtitle = "(most states based on FULLY vaccinated only; CA, GA, IA, MI & TX based on total doses administered) 
       Data via Centres for Disease Control, COVID Act Now, state health depts Graph by Charles Gaba / ACASignups.net",x = "2020 Trump Vote %", y = "% of Total Population Vaccinated")+
  theme_bw()+
  annotate("text", x = 0.18,y = 0.89, label = "Herd Immunity Threshold (?)", fontface = 'italic',color = 'blue',size = 3) +
  annotate("text", x = 0.13, y = 0.56, label = "TARGET:53.9%", fontface = 'italic', color = 'blue',size = 3) +
  annotate("text", x = 0.13, y = 0.525, label = "ACTUAL:50.8%", fontface = 'italic', color = 'blue',size = 3)+
  annotate("text", x = 0.4, y = 0.1, label = "09/04/2021", fontface = 'italic', color = 'red') +
  annotate("text", x = 0.2, y = 0.1, label = "y = -0.4224x + 0.6451", color = 'red',size = 3) +
  annotate("text", x = 0.2, y = 0.06, label = deparse(bquote(~R^2 ==~ 0.4216)),
            parse = T, size = 3, color = 'red')+
  annotate("text", x = 0.45, y = 1, label = "EVERY US COUNTY", size = 6) +
...
election_data_raw <- selected_data %>% 
  mutate (number_of_vote_union = samplesize*union/100, number_of_vote_spd = samplesize*spd/100, number_of_vote_af_d = samplesize*af_d/100, number_of_vote_fdp = samplesize*fdp/100, number_of_vote_linke = samplesize*linke/100, number_of_vote_grune = samplesize*grune/100) %>% 
  ...
  mutate (sum_vote_union = sum(number_of_vote_union), sum_vote_spd = sum(number_of_vote_spd), sum_vote_af_d = sum(number_of_vote_af_d), sum_vote_fdp = sum(number_of_vote_fdp), sum_vote_linke = sum(number_of_vote_linke), sum_vote_grune = sum(number_of_vote_grune)) %>% 
  mutate (average_rate_union = sum_vote_union/totalsample, average_rate_spd = sum_vote_spd/totalsample, average_rate_af_d = sum_vote_af_d/totalsample, average_rate_fdp = sum_vote_fdp/totalsample, average_rate_linke = sum_vote_linke/totalsample, average_rate_grune = sum_vote_grune/totalsample)
...
election_data = cbind(election_data,roll_mean_union=roll_mean_union,roll_mean_spd=roll_mean_spd,
                      roll_mean_af_d=roll_mean_af_d,roll_mean_fdp=roll_mean_fdp,
                      roll_mean_linke=roll_mean_linke,roll_mean_grune=roll_mean_grune)
...
election_data_raw <- selected_data %>% 
  ...
  #calculate the number of votes for each party (sample size*percentage of vote)
  group_by(end_date) %>% 
  mutate (totalsample = sum(samplesize)) %>% 
  ...
  #calculate the percentage of vote for each party
  mutate (rate_union = sum_vote_union/totalsample, 
          rate_spd = sum_vote_spd/totalsample, 
          rate_af_d = sum_vote_af_d/totalsample, 
          rate_fdp = sum_vote_fdp/totalsample, 
          rate_linke = sum_vote_linke/totalsample, 
          rate_grune = sum_vote_grune/totalsample) 
...
```{r,challenge2}
#calculate the 14-day moving average of vote for each party

election_data <- na.omit(election_data)


roll_mean_election_data <- election_data %>% 
  select(end_date, rate_union, rate_spd, rate_af_d, rate_fdp, rate_linke, rate_grune) 

alldates = seq(min(roll_mean_election_data$end_date), max(roll_mean_election_data$end_date), 1)
```

```{r}
alldates <- enframe(alldates, name= "name", value ="end_date")
```

```{r,challenge2}
roll_mean <- left_join (alldates, roll_mean_election_data, by = "end_date") %>% 
  fill(rate_union, rate_spd, rate_af_d, rate_fdp, rate_linke, rate_grune) %>% 
  mutate(
        average_rate_union = zoo::rollmean(rate_union, k=14, fill=NA),
        average_rate_spd = zoo::rollmean(rate_spd, k=14, fill = NA),
        average_rate_af_d = zoo::rollmean(rate_af_d, k=14, fill = NA),
        average_rate_fdp = zoo::rollmean(rate_fdp, k=14, fill = NA),
        average_rate_linke = zoo::rollmean(rate_linke, k=14, fill = NA),
        average_rate_grune = zoo::rollmean(rate_grune, k=14, fill = NA))
...
roll_mean %>% 
  ggplot(aes(x = end_date))+
  geom_line(aes(y = average_rate_union), color = "black") + 
  geom_line(aes(y = average_rate_spd), color="red3") +
  geom_line(aes(y = average_rate_af_d), color="midnightblue")+
  geom_line(aes(y = average_rate_fdp), color="orange1")+
  geom_line(aes(y = average_rate_linke), color="mediumvioletred")+
  geom_line(aes(y = average_rate_grune), color="green")+
  labs(title = "German Election Poll", x="Date", y="Percentage of vote")+
  theme(legend.position = "top",legend.background = element_rect(fill="lightblue", 
                                  size=0.5, linetype="solid"))
